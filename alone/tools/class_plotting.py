...
#def individual_energy_plot(runs):
def individual_energy_plot(values):
 store_static = []
 store_momentum= []
 store_static = values[0]
 store_momentum = values[1]
 if(len(values) > 2 ) : store_static_hard = values[2]
# for k in range(len(runs[0].runs)):
 print("test")
 print(len(store_static))
 for k in range(len(store_static)):
  print(k)
#  print(runs[0].runs[0])
#  nucleus = [runs[0].runs[k],runs[1].runs[k]]
#  nucleus = [runs[0].runs[k]]
  nucleus = [store_static[k],store_momentum[k]] 
  if(len(values) > 2 ) : nucleus = [store_static[k],store_momentum[k],store_static_hard[k]]
  fig = plt.figure(figsize=(19.2,10.8), dpi=100)                                                                                                                                              
  plt.suptitle(nucleus[0].sup_title) 
  gs = gridspec.GridSpec(2,3,hspace=0.4,wspace=0.3)          
  cols = 3
  for j in range(len(nucleus)): 
#   if (j == 1 ) : 
#      for l in range(len(nucleus[j].energies[1])):
#        nucleus[j].energies[1][l] = nucleus[j].energies[1][l] + store_momentum[6].energies[4][l] 
   for i in range(6):
        row = (i // cols)
        col = i % cols
        ax = fig.add_subplot(gs[row,col]) 
        ...
        ax.plot(nucleus[j].time,nucleus[j].energies[i],label = nucleus[j].plot_aspect.label ,
         				      ...
         				      markevery = nucleus[j].plot_aspect.markevery)
        if(i ==0) : ax.legend(loc='upper left',bbox_to_anchor=(-0.1,1.35))
#  if(k==0) : plt.savefig('output/all.png') 
  if(k==0) : plt.savefig('output/all.png') 
  if(k==1) : plt.savefig('output/potential.png') 
  if(k==2) : plt.savefig('output/coulomb.png') 
  if(k==3) : plt.savefig('output/kinetic.png') 
  if(k==4) : plt.savefig('output/asymmetry.png') 
  if(k==5) : plt.savefig('output/momentum_old.png') 
  if(k==6) : plt.savefig('output/mometum_opt.png') 

  fig = plt.figure(figsize=(19.2,10.8), dpi=100)                                                                                                                                              
  plt.suptitle(nucleus[0].sup_title) 
  gs = gridspec.GridSpec(1,2,hspace=0.4,wspace=0.3)
  cols = 2
  for j in range(len(nucleus)):
    for i in range(2):
        row = (i // cols)
        ...
        ax.set_xlabel(nucleus[j].time_label)
        ax.set_ylabel(nucleus[j].energy_label)
        ax.set_title(nucleus[j].titles[0])
	if(i==0) : 
	  ax.plot(nucleus[j].time,nucleus[j].energies[0],label = nucleus[j].plot_aspect.label ,
          				      color = nucleus[j].plot_aspect.color, 
          				      marker = nucleus[j].plot_aspect.marker, 
          				      linestyle = nucleus[j].plot_aspect.linestyle,
         				      markevery = nucleus[j].plot_aspect.markevery)
	if(i!=0):
	  if(k ==0) : 
	    y = nucleus[j].energies[0]
	    title = nucleus[j].titles[0]
	  if(k ==1) : 
#	    for l in range(len(nucleus[j].energies[1])):
#	      if(j ==1) : nucleus[j].energies[1][l] = nucleus[j].energies[1][l] + store_momentum[6].energies[4][l] 
	    y = nucleus[j].energies[1]
	    title = nucleus[j].titles[1]
	  if(k ==2) : 
	    y = nucleus[j].energies[3]
	    title = nucleus[j].titles[3]
	  if(k ==3) : 
	    y = nucleus[j].energies[2]
	    title = nucleus[j].titles[2]
	  if(k ==4) : 
	    y = nucleus[j].energies[5]
	    title = nucleus[j].titles[5]
	  if(k ==5) : 
	    y = nucleus[j].energies[4]
	    title = nucleus[j].titles[4]
	  if(k ==6) : 
	    y = nucleus[j].energies[4]
	    title = nucleus[j].titles[4]
	  ax.plot(nucleus[j].time,y,label = nucleus[j].plot_aspect.label ,
          				      color = nucleus[j].plot_aspect.color, 
          				      marker = nucleus[j].plot_aspect.marker, 
          				      linestyle = nucleus[j].plot_aspect.linestyle,
         				      markevery = nucleus[j].plot_aspect.markevery)
	  ...
	  ax.set_title(title)
        if(i ==0) : ax.legend(loc='upper left',bbox_to_anchor=(-0.1,1.1))
  if(k==0) : plt.savefig('output/all_reduce.png') 
  if(k==1) : plt.savefig('output/potential_reduce.png') 
  if(k==2) : plt.savefig('output/coulomb_reduce.png') 
  if(k==3) : plt.savefig('output/kinetic_reduce.png') 
  if(k==4) : plt.savefig('output/asymmetry_reduce.png') 
  if(k==5) : plt.savefig('output/momentum_old_reduce.png') 
  if(k==6) : plt.savefig('output/mometum_opt_reduce.png') 
